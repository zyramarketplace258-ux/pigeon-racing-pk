...
export default function ClubDetailPage() {
  ...
  const saveClubInfo = async () => {
    if (!club) return
    const trimName = editNameInput.trim()
    const trimCity = editCityInput.trim()
    if (!trimName) { setEditClubError('Club name is required'); return }
    if (!trimCity) { setEditClubError('City is required'); return }
    setEditClubError('')
    setSavingClub(true)
    ...
    <main className="min-h-screen bg-dark">
      <header className="bg-primary border-b border-secondary px-6 py-3 relative">
        ...
        <div className="text-center">
          ...
          <h1 className="text-sm font-bold text-secondary leading-tight mt-0.5">{club?.nameUrdu || club?.name}</h1>
          <p className="text-green-400 text-xs">{club?.cityUrdu || club?.city}</p>
        </div>
      </header>
        ...
        <div className="bg-surface border border-green-800 rounded-xl p-5 mb-8">
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-white font-bold text-lg">{club?.nameUrdu || club?.name}</h2>
                {club?.name && club?.nameUrdu && <p className="text-green-500 text-xs font-mono">{club.name}</p>}
                <p className="text-green-400 text-sm">{club?.cityUrdu || club?.city}</p>
            ...
            <div className="flex gap-2 flex-wrap">
              <button
                onClick={() => { setEditNameInput(club?.name || club?.nameUrdu || ''); setEditCityInput(club?.city || club?.cityUrdu || ''); setEditClubError(''); setEditingClub(v => !v) }}
          ...
          {editingClub && (
            <div className="mt-4 pt-4 border-t border-green-800 grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label className="text-green-400 text-xs mb-1 block">Club Name</label>
                <p className="text-green-600 text-xs mb-2">Type in English or Urdu — detected automatically</p>
                <input
                  value={editNameInput}
                  onChange={e => { setEditNameInput(e.target.value); setEditClubError('') }}
                  dir={isUrduScript(editNameInput) ? 'rtl' : 'ltr'}
                ...
                />
                {isUrduScript(editNameInput) && (
                  <p className="text-green-500 text-xs mt-1">Urdu detected ✓</p>
                )}
              </div>
              <div>
                <label className="text-green-400 text-xs mb-1 block">City</label>
                <p className="text-green-600 text-xs mb-2">Type in English or Urdu — detected automatically</p>
                <input
                  value={editCityInput}
                  onChange={e => { setEditCityInput(e.target.value); setEditClubError('') }}
                  dir={isUrduScript(editCityInput) ? 'rtl' : 'ltr'}
                  placeholder="e.g. Lahore / لاہور"
                  className="w-full bg-primary border border-green-700 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-secondary"
                />
                {isUrduScript(editCityInput) && (
                  <p className="text-green-500 text-xs mt-1">Urdu detected ✓</p>
                )}
              </div>
