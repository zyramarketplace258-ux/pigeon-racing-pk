...
export async function GET(request: NextRequest) {
  try {
    ...
    const watermarkCol = dark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.04)'
    ...
    if (photoUrl) {
      try {
        ...
        photoData = `data:${mime};base64,${btoa(b64)}`
      } catch { /* skip photo */ }
    }
      ...
      (
        <div style={{ display: 'flex', flexDirection: 'column', width: '100%', height: '100%', fontFamily: 'sans-serif' }}>
          ...
          <div style={{
            ...
            {/* Left rank stripe */}
            <div style={{ display: 'flex', width: 8, background: rankGrad, flexShrink: 0 }} />

            {/* Content */}
            <div style={{
              display: 'flex', flex: 1, position: 'relative',
              padding: '18px 32px 18px 16px', gap: 18, alignItems: 'center',
            }}>
              {/* Watermark */}
              <div style={{
                display: 'flex', position: 'absolute', right: 32, top: 4,
                color: watermarkCol, fontSize: 160, fontWeight: 900, lineHeight: 1,
              }}>
                {String(rank)}
              </div>
              ...
              {/* Avatar */}
              <div style={{
                display: 'flex', width: 80, height: 80, borderRadius: 40, flexShrink: 0,
                background: rankGrad, alignItems: 'center', justifyContent: 'center',
                border: `3px solid ${rankColor}`,
              }}>
                {photoData
                  ? <img src={photoData} width={80} height={80} style={{ borderRadius: 40, objectFit: 'cover' }} alt="" />
                  : <span style={{ color: 'white', fontSize: 30, fontWeight: 800 }}>{player.charAt(0).toUpperCase()}</span>
                }
              </div>

              {/* Name + badge */}
              <div style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
                <span style={{ color: nameCol, fontSize: 24, fontWeight: 800 }}>{player}</span>
                {area ? <span style={{ color: areaCol, fontSize: 13, marginTop: 2 }}>{area}</span> : null}
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8 }}>
                  <span style={{ fontSize: 18 }}>{medal}</span>
                  <span style={{ display: 'flex', background: rankGrad, color: badgeTextCol, fontSize: 11, fontWeight: 800, padding: '4px 12px', borderRadius: 20 }}>
                    {rankLabel}
                  </span>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', flexShrink: 0 }}>
                <span style={{ color: labelCol, fontSize: 9, letterSpacing: 2, textTransform: 'uppercase' }}>TOTAL SCORE</span>
                <span style={{ color: scoreCol, fontSize: 52, fontWeight: 900, lineHeight: 1, marginTop: 4 }}>{score}</span>
                <span style={{ color: labelCol, fontSize: 10, marginTop: 2 }}>hours flown</span>
              </div>
            </div>
          </div>

          {/* ── Timeline ── */}
          <div style={{ display: 'flex', flexDirection: 'column', padding: '12px 36px 8px', flex: 1, background: timelineBg }}>
            <span style={{ color: labelCol, fontSize: 9, letterSpacing: 3, textTransform: 'uppercase', fontWeight: 700, marginBottom: 12 }}>
              {filledCount > 0 ? `PIGEON TIMES  ·  ${filledCount} / ${times.length} LANDED` : 'PIGEON TIMES'}
            </span>

            <div style={{ display: 'flex', position: 'relative', flex: 1, alignItems: 'flex-start' }}>
              {/* Connector line */}
              <div style={{ display: 'flex', position: 'absolute', left: 6, top: 5, width: '95%', height: 2, background: lineCol }} />

              {times.slice(0, 9).map((time, i) => (
                <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                  <div style={{
                    display: 'flex', width: 12, height: 12, borderRadius: 6,
                    background: time ? rankColor : dotEmptyCol,
                  }} />
                  <span style={{ color: labelCol, fontSize: 9, marginTop: 4 }}>#{i + 1}</span>
          ...
          <div style={{
            ...
            <span style={{ color: '#81c784', fontSize: 11 }}>Love for the Loft</span>
          </div>
    ...
    return new Response(`Card error: ${err}`, {
      status: 500,
